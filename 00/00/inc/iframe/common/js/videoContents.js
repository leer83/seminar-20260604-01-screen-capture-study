...
var videoPlayer = function videoPlayer(elem) {
	...
	this.init = function () {
		...
		self.s_bar.on('mousedown mousemove mouseup mouseleave', self.soundControl);
		self.p_bar.on('mousedown mousemove mouseup mouseleave', self.progressControl);
		self.fullbtn.on('click', self.fullScrren)

		self.s_icon.on('click', function () {
			$(this).parent().find('.soundbox').toggle();
		...
		self.wrap.parent('.pop').find('.close').on('click', function () {
			console.log('?')
			self.stop();
	...
	this.soundControl = function (e) {
		...
		function volumeC() {
			if (volume < 1) {
				...
				self.video[0].muted = true;
				self.video[0].volume = 0;
				self.s_bar.find('span').height('0')
			} else if(volume > 100){
				self.video[0].volume = 1;
				self.s_bar.find('span').height('100%');
			} else {
				self.s_icon.removeClass('mute')
				self.video[0].muted = false;
				self.video[0].volume = volume / 100;
				self.s_bar.find('span').height(volume + '%')
			}
		}
