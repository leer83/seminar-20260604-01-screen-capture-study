 ...
 */
var lineContents = function lineContents(wrap, type) {
    var self = this;
    this.wrap = wrap;
        ...
        self.btnConfirm.off('click').on('click', function () {
            ...
            if (self.nlc.com() > 0) {
                for (var i = 0; i < chkBox.length; i++) {
                    ...
                    if (line_ans[1] != self.nlc.connections[i]) lineCon_ans = false;
                }
                // 정답
                if (lineCon_ans) {
                    ado_stop();
    ...
    this.makeLineConnector = function () {
        ...
        self.nlc = new LineConnector(leftDots, rightDots);
        self.nlc.chkConnAns = self.wrap.is('[data-chk-conn-ans]');
        self.nlc.onConnect = function (curIndex, curDot, connDot, isConnAns, connectAll) {
            if (self.nlc.chkConnAns) {
                ...
                if (isConnAns === true) {
                    ...
                    if (typeof self.onCorrect !== 'undefined') { self.onCorrect(self, curIndex, curDot, connDot, isConnAns, connectAll); }
                }
                else if (isConnAns === false) {

                    if (typeof self.onWrong !== 'undefined') { self.onWrong(self, curIndex, curDot, connDot, isConnAns); }
                }
                else {
    ...
    this.multiLineCon = function () {
        ...
        self.btnConfirm.off('click').on('click', function () {
            ...
            /*
            self.mlc.connections : 좌변점이 연결된 우변 점의 idx값 목록
            ...
            */
            ...
            //console.log('좌변점이 연결된 우변 점의 idx값 목록: ', self.mlc.connections);
    ...
    this.makeMultiLineConnector = function () {
        ...
        self.mlc = new MultiLineConnector(leftDots, rightDots);
        ...
        self.mlc.onConnect = function (curIndex, curDot, connDot, isConnAns, removeDup) {
            ...
            if (self.mlc.chkConnAns) {
                ...
                if (isConnAns === true && removeDup === false) {
                    ...
                    if (typeof self.onCorrect !== 'undefined') { self.onCorrect(self, curIndex, curDot, connDot, isConnAns, removeDup); }
                }
                else if (isConnAns === false) {
...
(function () {
    ...
    function LineConnector(leftItems, rightItems) {
        ...
        this.colorLine = '#FF261F'; // 선잇기 색상
        this.colorAns = '#FF261F';  // 정답선 색상

        this.chkConnAns = false;
            ...
            $(leftItem).off(downEvent).on(downEvent, function (e) {
                ...
                $(document).off(upEvent).on(upEvent, function (e) {
                    ...
                    if (rightItem && cline) {
                        ...
                        if (self.onConnect) {
                            if (self.chkConnAns) {
                                if(self.connections.length == self.leftItems.length){
                                    connectAll = true;
                                }
...
(function () {
    ...
    function MultiLineConnector(leftItems, rightItems) {
        getScale();
        this.leftItems = leftItems;
        this.rightItems = rightItems;
    ...
    p.setHandler = function () {
        ...
        for (var i = 0; i < self.leftItems.length; i++) {
            var leftItem = self.leftItems[i];
            leftItem.ondragstart = function () { return false; };
            var mx, my;
            $(leftItem).off(downEvent).on(downEvent, function (e) {
                getScale();
                mx = isMobile ? e.originalEvent.touches[0].pageX : e.pageX;
                ...
                var line = self.createLine(this.ox, this.oy, this.ox, this.oy, self.colorLine, 5);
        ...
        for (var i = 0; i < self.rightItems.length; i++) {
            var rightItem = self.rightItems[i];
            rightItem.ondragstart = function () { return false; };
    ...
    p.autoDrawLines = function (linevalue) {
        ...
        for (var j = 0; j < ans.length; j++) {
            if (ans[j]) {
                var dot = ans[j].split('_');
                var line = self.createLine(self.leftItems[dot[0]].ox, self.leftItems[dot[0]].oy, self.rightItems[dot[1]].ox, self.rightItems[dot[1]].oy, self.colorAns, 5);
                $(line).attr('li', dot[0]).attr('ri', dot[1]);
                self.lines.push(line);
                self.svg.appendChild(line);
            }
